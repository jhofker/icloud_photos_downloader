...
(keep-icloud-album-parameter)=
`--keep-icloud-album X`
    
:   Assets that would otherwise be deleted by [`--keep-icloud-recent-days`](keep-icloud-recent-days-parameter) will not be deleted in iCloud if they are in the specified album.
